...
RLS_TABLES = [
    ("courses", "organization_id", True, False),
]
...
def _policy_sql(table: str, org_column: str, nullable_org: bool) -> str:
    ...
    return f"""
    CREATE POLICY tenant_isolation_{table} ON course.{table}
...
CREATE_RLS = "\n".join(
    f"""
    ALTER TABLE course.{table} ENABLE ROW LEVEL SECURITY;
    {"ALTER TABLE course." + table + " FORCE ROW LEVEL SECURITY;" if force else ""}
...
DROP_RLS = "\n".join(
    f"""
    DROP POLICY IF EXISTS tenant_isolation_{table} ON course.{table};
    ALTER TABLE course.{table} NO FORCE ROW LEVEL SECURITY;
    ALTER TABLE course.{table} DISABLE ROW LEVEL SECURITY;
    """
