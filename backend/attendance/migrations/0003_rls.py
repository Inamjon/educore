...
RLS_TABLES = [
    ("attendances", "organization_id", True, False),
]
...
def _policy_sql(table: str, org_column: str, nullable_org: bool) -> str:
    ...
    return f"""
    CREATE POLICY tenant_isolation_{table} ON attendance.{table}
...
CREATE_RLS = "\n".join(
    f"""
    ALTER TABLE attendance.{table} ENABLE ROW LEVEL SECURITY;
    {"ALTER TABLE attendance." + table + " FORCE ROW LEVEL SECURITY;" if force else ""}
...
DROP_RLS = "\n".join(
    f"""
    DROP POLICY IF EXISTS tenant_isolation_{table} ON attendance.{table};
    ALTER TABLE attendance.{table} NO FORCE ROW LEVEL SECURITY;
