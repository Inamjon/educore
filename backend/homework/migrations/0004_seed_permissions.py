...
def seed_permissions_and_backfill_roles(apps, schema_editor):
    ...
    the (now assignments/submissions-inclusive) Permission catalog and
    re-runs provision_default_roles() for every existing org so their
    already-provisioned roles pick up the new grants. Idempotent
...
def unseed_permissions(apps, schema_editor):
    Permission = apps.get_model("foundation", "Permission")
    Permission.objects.filter(module__in=["assignments", "submissions"]).delete()
