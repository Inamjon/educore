...
def _make_org():
    return Organization.objects.create(
        name="Org", slug=f"org-hw-test-{uuid.uuid4().hex[:8]}", email="a@example.com"
    )
...
def test_teacher_cannot_create_assignment_for_another_teachers_group():
    ...
    teacher_a = TeacherProfile.objects.create(organization=org, user=teacher_a_user, teacher_code="TCH-A")
    teacher_b_user = _make_login(org, "+998900300002", "teacher")
    TeacherProfile.objects.create(organization=org, user=teacher_b_user, teacher_code="TCH-B")
...
def test_student_cannot_submit_on_behalf_of_another_student():
    ...
    teacher = TeacherProfile.objects.create(organization=org, user=teacher_user, teacher_code="TCH-C")
    group = _make_group(org, teacher, "GRP-C")

    student_a_user = _make_login(org, "+998900300004", "student")
    student_a = StudentProfile.objects.create(organization=org, user=student_a_user, student_code="STU-A")
    student_b_user = _make_login(org, "+998900300005", "student")
    student_b = StudentProfile.objects.create(organization=org, user=student_b_user, student_code="STU-B")
    GroupMember.objects.create(organization=org, group=group, student_profile=student_a)
    GroupMember.objects.create(organization=org, group=group, student_profile=student_b)

    assignment = Assignment.objects.create(organization=org, group=group, title="HW1", due_date="2026-09-10")
...
def test_student_cannot_see_classmates_submissions():
    ...
    teacher = TeacherProfile.objects.create(organization=org, user=teacher_user, teacher_code="TCH-D")
    group = _make_group(org, teacher, "GRP-D")

    student_a_user = _make_login(org, "+998900300007", "student")
    student_a = StudentProfile.objects.create(organization=org, user=student_a_user, student_code="STU-C")
    student_b_user = _make_login(org, "+998900300008", "student")
    student_b = StudentProfile.objects.create(organization=org, user=student_b_user, student_code="STU-D")

    assignment = Assignment.objects.create(organization=org, group=group, title="HW1", due_date="2026-09-10")
    Submission.objects.create(organization=org, assignment=assignment, student_profile=student_a, content="A's answer")
    submission_b = Submission.objects.create(organization=org, assignment=assignment, student_profile=student_b, content="B's answer")
...
def test_student_cannot_edit_after_grading():
    ...
    teacher = TeacherProfile.objects.create(organization=org, user=teacher_user, teacher_code="TCH-E")
    group = _make_group(org, teacher, "GRP-E")

    student_user = _make_login(org, "+998900300010", "student")
    student = StudentProfile.objects.create(organization=org, user=student_user, student_code="STU-E")
    assignment = Assignment.objects.create(organization=org, group=group, title="HW1", due_date="2026-09-10")
...
def test_teacher_grading_stamps_graded_by_and_graded_at():
    ...
    teacher = TeacherProfile.objects.create(organization=org, user=teacher_user, teacher_code="TCH-F")
    group = _make_group(org, teacher, "GRP-F")

    student_user = _make_login(org, "+998900300012", "student")
    student = StudentProfile.objects.create(organization=org, user=student_user, student_code="STU-F")
    assignment = Assignment.objects.create(organization=org, group=group, title="HW1", due_date="2026-09-10", max_score=100)
    submission = Submission.objects.create(organization=org, assignment=assignment, student_profile=student, content="answer")
...
def test_score_cannot_exceed_max_score():
    ...
    teacher = TeacherProfile.objects.create(organization=org, user=teacher_user, teacher_code="TCH-G")
    group = _make_group(org, teacher, "GRP-G")
    student_user = _make_login(org, "+998900300014", "student")
    student = StudentProfile.objects.create(organization=org, user=student_user, student_code="STU-G")
    assignment = Assignment.objects.create(organization=org, group=group, title="HW1", due_date="2026-09-10", max_score=50)
    submission = Submission.objects.create(organization=org, assignment=assignment, student_profile=student, content="answer")
...
def test_duplicate_submission_is_rejected():
    ...
    teacher = TeacherProfile.objects.create(organization=org, user=teacher_user, teacher_code="TCH-H")
    group = _make_group(org, teacher, "GRP-H")
    student_user = _make_login(org, "+998900300016", "student")
    student = StudentProfile.objects.create(organization=org, user=student_user, student_code="STU-H")
    assignment = Assignment.objects.create(organization=org, group=group, title="HW1", due_date="2026-09-10")
    Submission.objects.create(organization=org, assignment=assignment, student_profile=student, content="first")

    with pytest.raises(IntegrityError):
        Submission.objects.create(organization=org, assignment=assignment, student_profile=student, content="second")


def test_soft_delete_never_hard_deletes():
    org = _make_org()
    teacher_user = _make_login(org, "+998900300017", "teacher")
    teacher = TeacherProfile.objects.create(organization=org, user=teacher_user, teacher_code="TCH-I")
    ...
    assert Assignment.all_objects.filter(id=assignment_id).exists()
    assert Assignment.all_objects.get(id=assignment_id).deleted_at is not None
