...
def seed_permissions_and_backfill_roles(apps, schema_editor):
    ...
    BUG FIXED HERE (2026-08-12): the org backfill loop used to call
    ...
    granted payment_gateways to a single pre-existing org's center_admin
    role, despite `manage.py migrate` reporting success (the Permission
    catalog upsert above has no RLS, so it always succeeds regardless).
    See teacher/migrations/0006_seed_teacher_salary_permission.py's
    docstring for the full investigation. Fixed the same way here: read
    orgs through auth_bypass_rls (BYPASSRLS, sees everything) and apply
    real org context before each org's provision_default_roles() call.
